...
public class ImageManager implements Listener {
     ...
     */
    public void init() {
        Bukkit.getServer().getPluginManager().registerEvents(this, OllisCustomMaps.getPlugin(OllisCustomMaps.class));
        loadImages();
    }
    ...
    public void onMapInitEvent(MapInitializeEvent event) {
        if (hasImage(event.getMap().getId())) {
            ...
            view.addRenderer(new PaintRenderer(getImage(view.getId())));
    ...
    class CustomFile {
        ...
        private final OllisCustomMaps plugin = OllisCustomMaps.getPlugin(OllisCustomMaps.class);
